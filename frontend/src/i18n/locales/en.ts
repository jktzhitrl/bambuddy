export default {
  ...
  nav: {
    ...
    installAppSuccess: 'Druckübersicht was installed',
  },
  ...
  printers: {
    ...
    maintenance: {
      ...
      editFieldHelp: 'When on, this printer is paused from MQTT, queue dispatch and notifications — useful for repair, parallel Druckübersicht installs, or temporary suspension.',
    },
    // Chamber light
  ...
  archives: {
    ...
    no3mfBanner: {
      ...
      body: 'The slicer didn\'t leave the .gcode.3mf on the printer\'s SD card, so Druckübersicht couldn\'t pull the thumbnail or slicer metadata. This is usually because "Store sent files on external storage" is off in the slicer (Bambu Studio / OrcaSlicer Device tab).',
      ...
      bodyInternalStorage: 'Bambu Studio put the sliced file on the printer\'s internal storage instead of the card, so there was nothing for Druckübersicht to read over FTP. On H2-series and P2S its Print button always does that — only Send offers a choice, and that defaults to Cache too. Those prints are still archived with their name and timing, just without a thumbnail or slicer metadata. For complete archives, start the print from Druckübersicht, or slice in OrcaSlicer — or in Bambu Studio use Send with External selected and start the print afterwards. All of them need a card or stick in the printer.',
      titleNoExternalStorage: 'Some recent prints couldn\'t be archived — no storage in the printer',
      bodyNoExternalStorage: 'The printer reports no card or stick in its slot, so the sliced file had nowhere to land and Druckübersicht had nothing to read. Insert one and the next print will archive in full.',
      titleInternalHistory: 'Some recent prints were started from a file already on the printer',
      bodyInternalHistory: 'Those prints ran from the printer\'s own library — a re-print from its screen, a start from Handy, or a file sent earlier and printed later. Druckübersicht reads print files over FTP, which serves only the card or stick, while the printer keeps that library in an area FTP cannot reach, so there was no 3MF to read. No slicer setting changes this, because nothing was sent for these prints. They are still archived with their name and timing, and Edit Archive lets you fill in the filament used by hand. For a complete archive, start the print from Druckübersicht or from your slicer instead.',
    },
  ...
  queue: {
    ...
    batchOrders: {
      ...
      strandedPlate: 'Nothing left to copy settings from',
      strandedNotice: 'Cannot queue {{runs}} of the {{owed}} runs still owed: that plate has no queued or finished run left to copy settings from. Queue the plate once from the file, or cancel the order.',
    ...
    cancelBatchConfirmMessage: 'Close this order? Any runs still queued are cancelled, and it stops asking for what it has not produced.',
    batch: {
      defaultName: 'Batch',
      label: '{{count}} item',
    ...
    toast: {
      ...
      keptForOrder: 'Cancelled instead of removed: this is the only run its batch order can re-queue this plate from',
      historyKeptForOrders: 'Kept {{kept}} that batch orders still need.',
  ...
  settings: {
    ...
    preheatFilamentTargetsHint: 'Druckübersicht picks the highest target across the loaded AMS slots; PLA-only prints derive 0 and skip the chamber phase automatically.',
    ...
    authDisabledMessage: 'Enable authentication to create user accounts, manage permissions, and secure your Druckübersicht instance.',
    ...
    disableAuthenticationMessage: 'Are you sure you want to disable authentication? This will make your Druckübersicht instance accessible without login. All users will remain in the database but authentication will be disabled.',
    disableAuthentication: 'Disable Authentication',
    // Additional settings
    configureBambuddy: 'Configure Druckübersicht',
    ...
    captureFinishPhotoDescription: 'Take a photo from printer camera when print completes. Druckübersicht records a brief timelapse during the print so the photo can be sourced from the moment before the bed drops; the timelapse file is kept if you enabled timelapse for this print, otherwise it is deleted automatically after the photo is captured.',
    finishPhotoRestorePlate: 'Restore plate for finish photo',
    finishPhotoRestorePlateDescription: 'The printer drops the build plate about 100 mm when a print ends, leaving the finished print below the camera\'s framing. Druckübersicht raises it back to just above the last printed layer, takes the photo, then lowers it again. Skipped when the print height is unknown or another job is queued.',
    ...
    externalUrlDescription: 'The external URL where Druckübersicht is accessible. Used for notification images and external integrations.',
    ...
    encryption: {
      ...
      notConfiguredDesc: 'TOTP secrets and OIDC client_secrets are stored in plaintext. Set MFA_ENCRYPTION_KEY or restart Druckübersicht with a writable data directory to auto-generate one.',
      ...
      migrationErrorWarning: '{{count}} legacy row(s) failed to re-encrypt at startup. Check server logs and restart Druckübersicht to retry.',
    },
  ...
  hmsErrors: {
    ...
    runoutSlotUnknown: 'Filament ran out and the print is paused. Druckübersicht could not determine which slot the printer now expects — check the printer screen for the requested slot.',
    actions: {
  ...
  login: {
    ...
    resetStep1: 'Contact your Druckübersicht administrator',
  ...
  // Setup page
  setup: {
    title: 'Druckübersicht Setup',
    subtitle: 'Configure authentication for your Druckübersicht instance',
  ...
  users: {
    ...
    subtitle: 'Manage users and their access to your Druckübersicht instance',
    backToSettings: 'Back to Settings',
    createUser: 'Create User',
    noPermission: 'You do not have permission to access this page.',
    admin: 'Admin',
  ...
  profiles: {
    ...
    login: {
      ...
      captchaBody: 'Bambu is challenging your network before it will accept a sign-in, and the challenge cannot be answered from Druckübersicht. Your email and password are not the problem. The block is tied to your public IP address and normally clears by itself within a few hours — retrying repeatedly makes it last longer. To sign in now, use an access token from a browser session instead.',
  ...
  inventory: {
    ...
    spoolmanMixedContentBody: 'Druckübersicht is served over HTTPS (via your reverse proxy), but your Spoolman URL is still plain HTTP. Browsers block mixed content for security, so the embedded Spoolman UI can\'t render. Spoolman needs to be reachable over HTTPS for this to work.',
    spoolmanMixedContentFixReverseProxy: 'Put Spoolman behind the same reverse proxy as Druckübersicht (Traefik / Nginx / Caddy) with HTTPS, then update the Spoolman URL in Settings to the new HTTPS address.',
  ...
  backup: {
    ...
    repoIsPublicWarning: 'Repository is PUBLIC. Druckübersicht backups include MQTT credentials, Home Assistant tokens, Prometheus tokens, your Bambu Cloud email, and printer access codes via K-profiles. Saving is blocked until you make the repository private in your provider\'s settings.',
    repoVisibilityUnknown: 'Could not determine repository visibility. Druckübersicht refuses to back up to anything not confirmed private; saving will be blocked.',
    ...
    appSettingsDescription: 'Druckübersicht configuration (complete database)',
    ...
    restoreFromGit: {
      ...
      reloadHint: 'Reload Druckübersicht so the restored data appears everywhere.',
      ...
      notes: {
        ...
        settingsMqttRelayFailed: 'MQTT settings restored, but the relay could not be reconnected - restart Druckübersicht',
    ...
    localBackupDescription: 'Create a complete backup of your Druckübersicht data including the database, archives, uploads, and all files.',
    ...
    backupRestoredRestart: 'Backup restored. Please restart Druckübersicht.',
    ...
    pathCheck: {
      title: 'Druckübersicht cannot write to this directory',
      howToFix: 'How to fix:',
      sandboxed: 'The Druckübersicht service cannot write to {{path}}. Its systemd unit runs with ProtectSystem=strict, which makes every directory outside the install, data and log directories read-only for the service - even one you can write to from your own shell.',
      read_only: '{{path}} is on a read-only filesystem.',
      permission_denied: 'Druckübersicht is not allowed to write to {{path}}. Check the directory owner and permissions.',
      ...
      error: 'Druckübersicht cannot write to {{path}}.',
      ephemeralTitle: 'These backups will not survive a container restart',
      container_ephemeral: '{{path}} is inside the Druckübersicht container, not on the host. Backups written there are lost when the container is recreated. Mount the directory from the host:',
    },
  ...
  kProfiles: {
    ...
    modal: {
      ...
      notesHelp: 'Notes are saved in Druckübersicht, not on the printer',
  ...
  virtualPrinter: {
    ...
    remoteInterface: {
      ...
      hint: 'Override the IP address advertised via SSDP and used in the TLS certificate. Useful when Druckübersicht has multiple network interfaces.',
    },
    mode: {
      title: 'Mode',
      archive: 'Archive',
    ...
    howItWorks: {
      ...
      step2: 'In Archive, Review, and Queue modes, use the "Send" button in your slicer to upload 3MF files to Druckübersicht. The slicer will show "Print success" — the file is stored, not printed.',
  ...
  locationHaSensors: {
    ...
    autoAdd: {
      ...
      confirmMessage: 'Druckübersicht also found other sensors for this location. Choose which ones to add too.',
    ...
    options: {
      ...
      pollIntervalHint: 'How often Druckübersicht polls Home Assistant and refreshes sensor values on screen. Minimum 60 seconds.',
  ...
  externalLinks: {
    ...
    systemPages: 'Druckübersicht pages',
  ...
  diagnostic: {
    ...
    check: {
      ...
      external_storage: {
        ...
        warn_internal_storage: 'The option is on and storage is present, but the last print still went to the printer\'s internal storage, which Druckübersicht cannot read. On H2-series and P2S, Bambu Studio\'s Print button always sends there whatever this option is set to. Prints archive with their name and timing, but without a thumbnail or slicer metadata. For complete archives, start prints from Druckübersicht or slice in OrcaSlicer — or in Bambu Studio use Send with External selected, then start the print.',
        warn_internal_history: 'The option is on and storage is present, but the last print ran from a file that was already on the printer — a re-print from its screen, a start from Handy, or a file sent earlier. That library sits on internal storage Druckübersicht cannot read, and no setting changes it, because nothing was sent for that print. It still archives with its name and timing, but without a thumbnail or slicer metadata. For complete archives, start prints from Druckübersicht or send them from the slicer.',
      },
      port_rtsps: {
      ...
      network_mode: {
        ...
        warn: 'Druckübersicht is running in Docker bridge networking. Printer discovery and the Virtual Printer need host network mode — recreate the container with "network_mode: host".',
        skip: 'Not running in Docker — not applicable.',
      },
      subnet: {
        title: 'Network subnet',
        pass: 'The printer and Druckübersicht are on the same subnet.',
        warn: 'The printer ({{printer_ip}}) and Druckübersicht ({{host_ip}}) are on different subnets. They may not reach each other unless routing between the subnets is configured.',
        skip: 'Subnet could not be determined — skipped.',
      },
      mqtt_auth: {
        title: 'Printer credentials',
        pass: 'The printer accepted the connection.',
        fail: 'The printer is reachable but Druckübersicht is not connected to it. The access code or serial number is most likely wrong — the access code changes every time LAN Only or Developer Mode is toggled, so re-copy it from the printer screen. A printer that is rebooting, or already at its limit of simultaneous connections, can look the same.',
        fail_auth_rejected: 'The printer refused Druckübersicht\'s credentials. The access code or serial number is wrong — the access code changes every time LAN Only or Developer Mode is toggled, so re-copy it from the printer screen and save it in the printer settings.',
  ...
  systemHealth: {
    ...
    signature: {
      'ftp-auth-rejected': {
        ...
        fix: 'Re-copy the access code from the printer screen (LAN settings) and update it in the printer\'s settings in Druckübersicht.',
      },
      'ftp-connection-timeout': {
        name: 'File-transfer connection timed out',
        cause: 'Druckübersicht could not reach the printer\'s file-transfer port (FTPS 990). The port is blocked, or the printer is off or on another subnet.',
        fix: 'Make sure nothing blocks port 990 between Druckübersicht and the printer, and that both are on the same network.',
      },
      'ftp-ssl-error': {
      ...
      'database-locked': {
        ...
        fix: 'Switch Druckübersicht to an external PostgreSQL database. See the PostgreSQL guide in the documentation.',
      },
    },
  },
  ...
  vpDiagnostic: {
    ...
    check: {
      ...
      running: {
        ...
        fail: 'The virtual printer is enabled but its services are not running. Check the Druckübersicht log — a bind IP conflict or a permission error usually stops them.',
      },
      bind_interface: {
      ...
      certificate: {
        ...
        fail: 'The TLS certificate for this virtual printer is missing. Check that the Druckübersicht data directory is writable.',
      },
    },
  },
  ...
  failureDetection: {
    ...
    externalUrlHint: 'The ML API fetches the camera snapshot by URL. Set the External URL in General settings so the ML API container can reach Druckübersicht.',
  ...
  makerworld: {
    title: 'MakerWorld',
    description: 'Paste a MakerWorld model URL to import and print it directly from Druckübersicht — without leaving for the Bambu Handy app.',
    ...
    signInExpiredTitle: 'Bambu Cloud sign-in expired',
    signInExpiredBody: 'You are still signed in to Druckübersicht, but Bambu Lab has stopped accepting the stored token, so downloads will fail. Sign in to Bambu Cloud again.',
    ...
    disclaimer: 'MakerWorld integration uses community-documented API endpoints. Druckübersicht is not affiliated with or endorsed by MakerWorld or Bambu Lab.',
  ...
  libraryPurge: {
    ...
    effect4: 'Files in external (linked) folders are skipped — Druckübersicht never deletes bytes it does not own.',
